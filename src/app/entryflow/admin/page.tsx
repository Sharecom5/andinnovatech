...
export default function SaaSAdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      fetch('/api/entryflow/settings').then(r => r.json()).then(d => setAllowMultipleEntry(d.allowMultipleEntry))
    }
  }, [status, router])

  async function saveSettings() {
    await fetch('/api/entryflow/settings', {
      method: 'PUT',
  ...
  async function handleBlast() {
    ...
    try {
      ...
      for (let c = 0; c < allRows.length; c += CHUNK_SIZE) {
        ...
        const res = await fetch('/api/entryflow/bulk-save', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
